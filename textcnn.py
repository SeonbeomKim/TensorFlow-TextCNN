...
class TextCNN:
	def __init__(self, sess, voca_size, embedding_mode, embedding_size, window_size, filters, num_classes, pad_idx, lr):
		self.sess = sess
		...
		with tf.name_scope("embedding_table"):
			if self.embedding_mode == 'rand':
				embedding_table = self.rand_embedding_table(self.voca_size, self.pad_idx, self.embedding_size)
			else:
				pass # 구현해야함.

		with tf.name_scope("embedding_lookup"): 
			embedding = tf.nn.embedding_lookup(embedding_table, self.idx_input) # [N, self.idx_input_length, self.embedding_size]
			self.embedding = tf.expand_dims(embedding, axis=-1) # [N, self.idx_input_length, self.embedding_size, 1]
			#multichannel이면 expand_dims 한것 두개 axis=-1에서 concat해야함 [N, self.idx_input_length, self.embedding_size, 2] 되도록 
