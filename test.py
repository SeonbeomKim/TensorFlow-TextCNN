...
voca_size = 10
embedding_mode='multichannel'#'nonstatic'#'static'#'rand'
embedding_size = 3
window_size = [3,4,5]
filters=2
num_classes = 2
...
word2vec = np.random.randn(voca_size, embedding_size)
...
model = textcnn.TextCNN(
		...
		word_embedding=word2vec # if embedding_mode is not 'rand'
	)

testcode1()
#print(sess.run(model.embedding_table))
print(sess.run(model.static_embedding_table))
print(sess.run(model.nonstatic_embedding_table))
print(word2vec, word2vec.shape)
